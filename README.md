...
**這把金鑰碰得到的就是本文件列的十支端點**（九支資料 API ＋ 一支萃取），沒有別的。
以下三類都要在產品介面做（或由租戶管理者以登入身分呼叫對應的端點）：

| 想做的事 | 這支 API | 要去哪裡做 |
|---|---|---|
| 建立租戶 | 碰不到 | 平台管理員在平台管理頁建立；租戶建好之後才會有金鑰可產生 |
| **QA 集**（一問一答、逐字直答用） | **建不出來、也寫不進去** | 產品介面的〈QA 集管理〉頁建立與匯入 |
| **術語表／別名表** | 碰不到 | 產品介面的〈術語表管理〉／〈別名表管理〉頁 |

### 為什麼對 QA 集打 `records`／`files` 會 409

`POST /api/v1/datasets` **沒有** `kind` 參數，所以它只建得出**檔案集**（本文件第二節那種）。
而對一個既有的 QA 集打 `POST …/records` 或 `POST …/files` 會被擋下（`409`，`code` 為
`use_qa_import`），這是刻意的：

QA 集的逐字直答靠**標題槽**做完全命中比對（問題進 `title`、答案進 `content`），而一般匯入的
列多半沒有 `title`。放行的話那些列**真的會進索引**，畫面上也會顯示「這個 QA 集有 N 筆資料」
——而逐字直答對它們**永遠不命中**，整條路一個訊號都沒有。擋在寫入那一刻，是唯一看得見的位置。

### 術語表與別名表怎麼上傳

**沒有「上傳一個檔」這條路**——它們不是匯入，是「編輯一張表然後存檔」：

| | 術語表 | 別名表 |
|---|---|---|
| 建表 | `POST /glossary`（`name` 必填，另有「什麼時候該查這張表」與工具提示詞） | `POST /alias` |
| 詞條 | `PUT /glossary/{id}/entries`，body ＝ `[{"term":…,"definition":…}, …]` | `PUT /alias/{id}/entries`，body ＝ `[{"canonical":…,"aliases":[…]}, …]` |
| 語意 | **整批取代**：送出去的就是這張表的**全部**詞條，沒送的等於刪掉 | 同左 |
| 認證 | 登入身分（`Authorization: Bearer`）＋ `data:write` 能力，**不是 `X-API-Key`** | 同左 |

⚠️ **`PUT` 是取代不是追加。** 要加 10 筆就得把「現有的全部 ＋ 這 10 筆」一起送——只送新的
那 10 筆，等於把整張表換成那 10 筆。介面上的〈管理詞條〉就是這個語意：開窗時讀回全部、
逐列編輯、存檔時整批送出。

兩件容易踩的：

- **每張表的詞條數有上限**，超過回 `400` 並說出「現在幾筆／上限多少」。那個數字是向引擎
  **現取**的、不是寫死在文件裡的，所以照回應說的做。
- **別名不住在術語表裡。** 術語表的詞條只有 `term`／`definition`，送 `aliases` 進去會 `422`
  ——刻意不靜默丟掉：丟掉的話畫面會顯示「已儲存」而別名不知去向。別名一律在別名表，
  而它套用在哪個階段（檢索／比對／推理／輸出）由對話設定決定，不標在每一列上。

### 那我的資料該放哪一種？

| 你的資料 | 放哪裡 |
|---|---|
| 文件、條文、產品說明、公告——「內容」型 | **檔案集**（本文件的主體：`records` 或 `files`） |
| 業務單位審定過的一問一答，希望**逐字**回覆 | QA 集（產品介面匯入） |
| 專有名詞的解釋、同義詞對照 | 術語表／別名表（產品介面） |

檔案集是問答檢索的知識庫，也是這支 API 的正常用途——**不需要為了「這是問答系統」而把內容
硬塞成一問一答**。
